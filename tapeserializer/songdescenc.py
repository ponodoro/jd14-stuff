...
print("JD2014 SONGDESC SERIALIZER")
...
title=unidecode(songdesc["COMPONENTS"][0]["Title"])
artist=unidecode(songdesc["COMPONENTS"][0]["Artist"])
...
enc.write(b'\x00\x00\x00\x05')
try:
    enc.write(struct.pack(">i",len(songdesc["COMPONENTS"][0]["RelatedAlbums"])))
    for relatedalbum in songdesc["COMPONENTS"][0]["RelatedAlbums"]:
        enc.write(struct.pack(">i",len(relatedalbum))+relatedalbum.encode())
    enc.write(b'\x00\x00\x00\x01\x00\x00\x00\x9C\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x03\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xFF\xFF\xFF\xFF\x00\x00\x00\x00\x00\x00\x00\x01')
except:
    enc.write(b'\x00\x00\x00\x00\x00\x00\x00\x01\x00\x00\x00\x9C\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x03\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xFF\xFF\xFF\xFF\x00\x00\x00\x00\x00\x00\x00\x01')
enc.write(struct.pack(">i",len(artist))+artist.encode())
enc.write(struct.pack(">i",len(title))+title.encode())
