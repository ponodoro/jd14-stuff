import os, json, re, struct, zlib, io
from unidecode import unidecode
print("JD2014 TAPE SERIALIZER")
...
try:
    ...
    json.dump(makelayerid,open('layerid.json',"w"))
...
#making a separated json layout to convert the values
try:
...
for dtape_clips in dtape["Clips"]:
    if dtape_clips["__class"] == "GoldEffectClip":
        ...
        elif dtape_clips["EffectType"]==2:#goldmovecascade is a gold move frame without the pre effect
            timelinetpl["blocks"].append({
    ...
    if dtape_clips["__class"]=="PictogramClip":
        ...
        "pictoname": pictogrampath.split("/")[-1].split(".")[0].upper(),
    ...
    if dtape_clips["__class"]=="MotionClip":
        if dtape_clips["ClassifierPath"].endswith('.msm'):
            ...
            "__class": "MoveClip",
            "moveName": msmpath.split("/")[-1].split(".")[0].upper(),
            "layerID": coachid,
        ...
        if dtape_clips["ClassifierPath"].endswith('.gesture'):
            ...
            "__class": "MoveClip",
            "moveName": gesturepath.split("/")[-1].split(".")[0].upper(),
            "layerID": coachid,
...
#modifying the msms because the current ones dont work on jd14 :(
try:
    for movemodel_clips in os.listdir("input/msm"):
        ...
        with open("input/msm/"+movemodel_clips,'rb')as f: # modifying the msms
            ...
            msmake.write(b'\x00\x00\x00\x00\x21\x1C\x00\x00\x00\x00\x00\x00\x00\x00\x00\x32')
...
try:
    for pictomodel_clips in os.listdir("input/picto"):

        with open("input/picto/"+pictomodel_clips,'rb')as f: 

            pictoblock_name=pictomodel_clips.replace(codename+'_','').replace(codenamelow+'_','').replace('.tga', '').replace('.png','').replace('.ckd','')
            
        timelinetpl["pictoModels"].append(
        {
        "__class": "TimelineModelPicto",
        "name": pictoblock_name.upper()
        })
...
for picto in timelinetpl["pictos"]:
    if picto["__class"]=="PictoClip":
        pictofile=picto["texturePath"].split("/")[-1]
        pictopath=picto["texturePath"].replace(pictofile,"")
        ...
        pict+=struct.pack(">I",len(pictopath))+pictopath.encode()
        pict+=struct.pack(">I",len(pictofile))+pictofile.encode()
        pict+=struct.pack("<I",zlib.crc32(pictofile.encode()))+struct.pack(">I",0)

pictomodel=struct.pack(">I",len(timelinetpl["pictoModels"]))
for pictmodel in timelinetpl["pictoModels"]:
    if pictmodel["__class"]=="TimelineModelPicto":
        pictomodel+=b'\x00\x00\x00\x1C'
        pictomodel+=struct.pack(">I",len(pictmodel["name"]))+pictmodel["name"].encode()

msm=struct.pack(">I",len(timelinetpl["moves"]))
for move in timelinetpl["moves"]:
    if move["__class"]=="MoveClip":
        movefile=move["classifierPath"].split("/")[-1]
        movepath=move["classifierPath"].replace(movefile,"")
        msm+=b'\x00\x00\x00\x9C'
        msm+=struct.pack(">I",len(move["moveName"]))+move["moveName"].encode()
        msm+=struct.pack(">I",move["layerID"])
        msm+=struct.pack(">I",len(movepath))+movepath.encode()
        msm+=struct.pack(">I",len(movefile))+movefile.encode()
        msm+=struct.pack("<I",zlib.crc32(movefile.encode()))+struct.pack(">I",0)
...
for kinectmove in timelinetpl["kinectmoves"]:
    if kinectmove["__class"]=="MoveClip":
        kinectmovefile=kinectmove["classifierPath"].split("/")[-1]
        kinectmovepath=kinectmove["classifierPath"].replace(movefile,"")
        ...
        gesture+=struct.pack(">I",len(kinectmovepath))+kinectmovepath.encode()
        gesture+=struct.pack(">I",len(kinectmovefile))+kinectmovefile.encode()
        gesture+=struct.pack("<I",zlib.crc32(kinectmovefile.encode()))+struct.pack(">I",0)
...
#intro part
tape_version=int((430*len(timelinetpl["pictos"]+timelinetpl["moves"]+timelinetpl["kinectmoves"]+timelinetpl["lyrics"]+timelinetpl["blocks"]+timelinetpl["pictoModels"]+timelinetpl["moveModels"]+timelinetpl["kinectmoveModels"]+timelinetpl["blockModels"]+timelinetpl["karaokedata"]))+166)
